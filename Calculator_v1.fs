...
module CalculatorImplementation =
    ...
    let addPendingMathOp services op state =
        maybe {
            let! currentNumber = state.display |> services.getDisplayNumber
...
module CalculatorUI =
    ...
    type CalculatorForm(initState: InitState, calculate: Calculate) as this =
        ...
        let mutable setDisplayedText = fun text -> () // do nothing
        ...
        let handleDigit digit = Digit digit |> handleInput

        let handleOp op = Op op |> handleInput

        let handleAction action = Action action |> handleInput
        ...
        member this.CreateButtons() =
            ...
            let sevenButton =
                new Button(Text = "7", Size = buttonSize, Location = getPos (1, 0), BackColor = DigitButtonColor)

            sevenButton |> addDigitButton Seven

            let eightButton =
                new Button(Text = "8", Size = buttonSize, Location = getPos (1, 1), BackColor = DigitButtonColor)

            eightButton |> addDigitButton Eight

            let nineButton =
                new Button(Text = "9", Size = buttonSize, Location = getPos (1, 2), BackColor = DigitButtonColor)

            nineButton |> addDigitButton Nine

            let clearButton =
                new Button(Text = "C", Size = buttonSize, Location = getPos (1, 3), BackColor = DangerButtonColor)

            clearButton |> addActionButton Clear

            let addButton =
                new Button(Text = "+", Size = doubleHeightSize, Location = getPos (1, 4), BackColor = OpButtonColor)

            addButton |> addOpButton Add

            let fourButton =
                new Button(Text = "4", Size = buttonSize, Location = getPos (2, 0), BackColor = DigitButtonColor)

            fourButton |> addDigitButton Four

            let fiveButton =
                new Button(Text = "5", Size = buttonSize, Location = getPos (2, 1), BackColor = DigitButtonColor)

            fiveButton |> addDigitButton Five

            let sixButton =
                new Button(Text = "6", Size = buttonSize, Location = getPos (2, 2), BackColor = DigitButtonColor)

            sixButton |> addDigitButton Six

            let divideButton =
                new Button(Text = "/", Size = buttonSize, Location = getPos (2, 3), BackColor = OpButtonColor)

            divideButton |> addOpButton Divide

            let oneButton =
                new Button(Text = "1", Size = buttonSize, Location = getPos (3, 0), BackColor = DigitButtonColor)

            oneButton |> addDigitButton One

            let twoButton =
                new Button(Text = "2", Size = buttonSize, Location = getPos (3, 1), BackColor = DigitButtonColor)

            twoButton |> addDigitButton Two

            let threeButton =
                new Button(Text = "3", Size = buttonSize, Location = getPos (3, 2), BackColor = DigitButtonColor)

            threeButton |> addDigitButton Three

            let multButton =
                new Button(Text = "*", Size = buttonSize, Location = getPos (3, 3), BackColor = OpButtonColor)

            multButton |> addOpButton Multiply

            let equalButton =
                new Button(Text = "=", Size = doubleHeightSize, Location = getPos (3, 4), BackColor = OpButtonColor)

            equalButton |> addActionButton Equals

            let zeroButton =
                new Button(Text = "0", Size = doubleWidthSize, Location = getPos (4, 0), BackColor = DigitButtonColor)
            ...
            let pointButton =
                new Button(
                    ...
                    Size = buttonSize,
                    Location = getPos (4, 2),
                    BackColor = DigitButtonColor
                )

            pointButton |> addDigitButton DecimalSeparator

            let minusButton =
                new Button(Text = "-", Size = buttonSize, Location = getPos (4, 3), BackColor = OpButtonColor)
